...
__BSS(RAM3) volatile uint8_t I2CMasterBuffer[BUFSIZE];
__BSS(RAM3) volatile uint8_t I2CSlaveBuffer[BUFSIZE];
...
uint32_t I2CRead(uint8_t address,uint8_t subaddress, uint8_t *buffer, uint8_t len)
{
	  I2CWriteLength = 2;
	  I2CReadLength = len;
	  I2CMasterBuffer[0] = address;
	  I2CMasterBuffer[1] = subaddress;
	  I2CMasterBuffer[2] = address | 1;
	  I2CEngine( );
	  memcpy(buffer,I2CSlaveBuffer,RdIndex);
	  return RdIndex;
