...
struct nfc_driver_list {
 ...
 const struct nfc_driver_list drv_pn532 = {
 		.driver = &pn532_spi_driver,
 		.next = NULL,
 };

 const struct nfc_driver_list *nfc_drivers = &drv_pn532;
...
nfc_register_driver(const struct nfc_driver *ndr)
{
  ...
  struct nfc_driver_list *pndl = (struct nfc_driver_list *)pvPortMalloc(sizeof(struct nfc_driver_list));
  if (!pndl)
    return NFC_ESOFT;
...
{
  nfc_device *pnd = NULL;
  pnd = nfc_drivers->driver->open(context, connstring);
  return pnd;
/*
  nfc_connstring ncs;
  if (connstring == NULL) {
                                   ...
                                   nfc_target ant[], const size_t szTargets)
{
  ...
  nfc_target *nt = pvPortMalloc(sizeof(nfc_target));
  if (nt==NULL)
	  return res;
