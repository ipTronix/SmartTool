...
__BSS(RAM3) BUFFER_DECLARE(abtPacketBuffer, PN53x_EXTENDED_FRAME__DATA_MAX_LEN);
__BSS(RAM3) BUFFER_DECLARE(abtPacketBuffer2, PN53x_EXTENDED_FRAME__DATA_MAX_LEN);
...
nfc_modulation_type pn53x_supported_modulation_as_initiator[9];
/* prototypes */
...
pn53x_init(struct nfc_device *pnd)
{
  ...
  if (!CHIP_DATA(pnd)->supported_modulation_as_initiator) {
    CHIP_DATA(pnd)->supported_modulation_as_initiator = pvPortMalloc(sizeof(nfc_modulation_type) * 9);
    if (! CHIP_DATA(pnd)->supported_modulation_as_initiator)
      return NFC_ESOFT;
#endif
...
pn53x_transceive(struct nfc_device *pnd, const uint8_t *pbtTx, const size_t szTx, uint8_t *pbtRx, const size_t szRxLen, int timeout)
{
  ...
  switch (pbtTx[0]) {
    ...
    case TgSetMetaData:
      if (pbtRx[0] & 0x80) { return NFC_ETGRELEASED;/*abort();*/ } // NAD detected
      if (pbtRx[0] & 0x40) { return NFC_ETGRELEASED;/*abort();*/ } // MI detected
...
pn53x_writeback_register(struct nfc_device *pnd)
{
  ...
  BUFFER_INIT(abtPacketBuffer, PN53x_EXTENDED_FRAME__DATA_MAX_LEN);
  BUFFER_APPEND(abtPacketBuffer, ReadRegister);
  ...
  for (size_t n = 0; n < PN53X_CACHE_REGISTER_SIZE; n++) {
    if ((CHIP_DATA(pnd)->wb_mask[n]) && (CHIP_DATA(pnd)->wb_mask[n] != 0xff)) {
      ...
      BUFFER_APPEND(abtPacketBuffer, pn53x_register_address  >> 8);
      BUFFER_APPEND(abtPacketBuffer, pn53x_register_address & 0xff);
    }
  }

  if (BUFFER_SIZE(abtPacketBuffer) > 1) {
    // It needs to read some registers
    //uint8_t abtRes[PN53x_EXTENDED_FRAME__DATA_MAX_LEN];
    //size_t szRes = sizeof(abtRes);
	// read and write using the same buffer. this works because data is written in received buffer after it has been sent
    // It transceives the previously constructed ReadRegister command
    if ((res = pn53x_transceive(pnd, abtPacketBuffer, BUFFER_SIZE(abtPacketBuffer), abtPacketBuffer, PN53x_EXTENDED_FRAME__DATA_MAX_LEN, -1)) < 0) {
      return res;
    }
    size_t i = 0;
    if (CHIP_DATA(pnd)->type == PN533) {
    ...
    for (size_t n = 0; n < PN53X_CACHE_REGISTER_SIZE; n++) {
      if ((CHIP_DATA(pnd)->wb_mask[n]) && (CHIP_DATA(pnd)->wb_mask[n] != 0xff)) {
        CHIP_DATA(pnd)->wb_data[n] = ((CHIP_DATA(pnd)->wb_data[n] & CHIP_DATA(pnd)->wb_mask[n]) | (abtPacketBuffer[i] & (~CHIP_DATA(pnd)->wb_mask[n])));
        if (CHIP_DATA(pnd)->wb_data[n] != abtPacketBuffer[i]) {
  ...
  BUFFER_INIT(abtPacketBuffer, PN53x_EXTENDED_FRAME__DATA_MAX_LEN);
  BUFFER_APPEND(abtPacketBuffer, WriteRegister);
  for (size_t n = 0; n < PN53X_CACHE_REGISTER_SIZE; n++) {
    if (CHIP_DATA(pnd)->wb_mask[n] == 0xff) {
      ...
      BUFFER_APPEND(abtPacketBuffer, pn53x_register_address  >> 8);
      BUFFER_APPEND(abtPacketBuffer, pn53x_register_address & 0xff);
      BUFFER_APPEND(abtPacketBuffer, CHIP_DATA(pnd)->wb_data[n]);
  ...
  if (BUFFER_SIZE(abtPacketBuffer) > 1) {
    ...
    if ((res = pn53x_transceive(pnd, abtPacketBuffer, BUFFER_SIZE(abtPacketBuffer), NULL, 0, -1)) < 0) {
      return res;
    }
  }
  return NFC_SUCCESS;
                                          ...
                                          int timeout)
{
  ...
  size_t  szTargetsData = PN53x_EXTENDED_FRAME__DATA_MAX_LEN;//sizeof(abtTargetsData);
  int res = 0;
  ...
  if (nm.nmt == NMT_ISO14443BI || nm.nmt == NMT_ISO14443B2SR || nm.nmt == NMT_ISO14443B2CT) {
    ...
    if (nm.nmt == NMT_ISO14443B2SR) {
      ...
      if ((res = pn53x_initiator_transceive_bytes(pnd, abtReqt, sizeof(abtReqt), abtPacketBuffer + 2, sizeof(abtPacketBuffer) - 2, timeout)) < 0) {
        return res;
      }
      szTargetsData = (size_t)res;
    }
    if ((res = pn53x_initiator_transceive_bytes(pnd, pbtInitData, szInitData, abtPacketBuffer, sizeof(abtPacketBuffer), timeout)) < 0) {
      return res;
    }
    szTargetsData = (size_t)res;
    if (nm.nmt == NMT_ISO14443B2CT) {
      ...
      if ((res = pn53x_initiator_transceive_bytes(pnd, abtRead, sizeof(abtRead), abtPacketBuffer + 4, sizeof(abtPacketBuffer) - 4, timeout)) < 0) {
        return res;
      }
    ...
    if (pnt) {
      ...
      if ((res = pn53x_decode_target_data(abtPacketBuffer, szTargetsData, CHIP_DATA(pnd)->type, nm.nmt, &(pnt->nti))) < 0) {
        return res;
      }
    }
    if (nm.nmt == NMT_ISO14443BI) {
      // Select tag
      uint8_t abtAttrib[6];
      memcpy(abtAttrib, abtPacketBuffer, sizeof(abtAttrib));
  ...
  if ((res = pn53x_InListPassiveTarget(pnd, pm, 1, pbtInitData, szInitData, abtPacketBuffer, &szTargetsData, timeout)) <= 0)
    return res;
  ...
  if (pnt) {
    pnt->nm = nm;
    // Fill the tag info struct with the values corresponding to this init modulation
    if ((res = pn53x_decode_target_data(abtPacketBuffer + 1, szTargetsData - 1, CHIP_DATA(pnd)->type, nm.nmt, &(pnt->nti))) < 0) {
      return res;
    }
                                ...
                                const uint8_t *pbtTxPar, uint8_t *pbtRx, uint8_t *pbtRxPar)
{
  ...
  abtPacketBuffer[0] = InCommunicateThru;
  // Check if we should prepare the parity bits ourself
  if (!pnd->bPar) {
    // Convert data with parity to a frame
    if ((res = pn53x_wrap_frame(pbtTx, szTxBits, pbtTxPar, abtPacketBuffer + 1)) < 0)
      return res;
    szFrameBits = res;
  } else {
    szFrameBits = szTxBits;
  ...
  if (pnd->bPar)
    memcpy(abtPacketBuffer + 1, pbtTx, szFrameBytes);
  ...
  size_t  szRx = PN53x_EXTENDED_FRAME__DATA_MAX_LEN;//sizeof(abtRx);
  if ((res = pn53x_transceive(pnd, abtPacketBuffer, szFrameBytes + 1, abtPacketBuffer2, szRx, -1)) < 0)
    return res;
  szRx = (size_t) res;
  ...
  if (pbtRx != NULL) {
    ...
    if (!pnd->bPar) {
      ...
      if ((res = pn53x_unwrap_frame(abtPacketBuffer2 + 1, szFrameBits, pbtRx, pbtRxPar)) < 0)
        return res;
      szRxBits = res;
    } else {
      ...
      memcpy(pbtRx, abtPacketBuffer2 + 1, szRx - 1);
    }
  }
                                 ...
                                 const size_t szRx, int timeout)
{
  ...
  if (pnd->bEasyFraming) {
    abtPacketBuffer[0] = InDataExchange;
    abtPacketBuffer[1] = 1;              /* target number */
    memcpy(abtPacketBuffer + 2, pbtTx, szTx);
    szExtraTxLen = 2;
  } else {
	  abtPacketBuffer[0] = InCommunicateThru;
    memcpy(abtPacketBuffer + 1, pbtTx, szTx);
    szExtraTxLen = 1;
  }
  ...
  //uint8_t  abtRx[PN53x_EXTENDED_FRAME__DATA_MAX_LEN];
  if ((res = pn53x_transceive(pnd, abtPacketBuffer, szTx + szExtraTxLen, abtPacketBuffer2, sizeof(abtPacketBuffer2), timeout)) < 0) {
    pnd->last_error = res;
    return pnd->last_error;
  }
  ...
  if (pbtRx != NULL) {
    ...
    // Copy the received bytes
    memcpy(pbtRx, abtPacketBuffer2 + 1, szRxLen);
  }
...
static uint32_t __pn53x_get_timer(struct nfc_device *pnd, const uint8_t last_cmd_byte)
{
  ...
  BUFFER_INIT(abtPacketBuffer, PN53x_EXTENDED_FRAME__DATA_MAX_LEN);
  BUFFER_APPEND(abtPacketBuffer, ReadRegister);
  BUFFER_APPEND(abtPacketBuffer, PN53X_REG_CIU_TCounterVal_hi  >> 8);
  BUFFER_APPEND(abtPacketBuffer, PN53X_REG_CIU_TCounterVal_hi & 0xff);
  BUFFER_APPEND(abtPacketBuffer, PN53X_REG_CIU_TCounterVal_lo  >> 8);
  BUFFER_APPEND(abtPacketBuffer, PN53X_REG_CIU_TCounterVal_lo & 0xff);
  //uint8_t abtRes[PN53x_EXTENDED_FRAME__DATA_MAX_LEN];
  size_t szRes = PN53x_EXTENDED_FRAME__DATA_MAX_LEN;//sizeof(abtRes);
  // Let's send the previously constructed ReadRegister command
  if (pn53x_transceive(pnd, abtPacketBuffer, BUFFER_SIZE(abtPacketBuffer), abtPacketBuffer2, szRes, -1) < 0) {
    return false;
  }
  counter_hi = abtPacketBuffer2[off];
  counter_lo = abtPacketBuffer2[off + 1];
  counter = counter_hi;
                                      ...
                                      const uint8_t *pbtTxPar, uint8_t *pbtRx, uint8_t *pbtRxPar, uint32_t *cycles)
{
  ...
  BUFFER_APPEND(abtPacketBuffer, PN53X_REG_CIU_Command  >> 8);
  BUFFER_APPEND(abtPacketBuffer, PN53X_REG_CIU_Command & 0xff);
  BUFFER_APPEND(abtPacketBuffer, SYMBOL_COMMAND & SYMBOL_COMMAND_TRANSCEIVE);
  BUFFER_APPEND(abtPacketBuffer, PN53X_REG_CIU_FIFOLevel  >> 8);
  BUFFER_APPEND(abtPacketBuffer, PN53X_REG_CIU_FIFOLevel & 0xff);
  BUFFER_APPEND(abtPacketBuffer, SYMBOL_FLUSH_BUFFER);
  for (i = 0; i < ((szTxBits / 8) + 1); i++) {
    BUFFER_APPEND(abtPacketBuffer, PN53X_REG_CIU_FIFOData  >> 8);
    BUFFER_APPEND(abtPacketBuffer, PN53X_REG_CIU_FIFOData & 0xff);
    BUFFER_APPEND(abtPacketBuffer, pbtTx[i]);
  }
  // Send data
  BUFFER_APPEND(abtPacketBuffer, PN53X_REG_CIU_BitFraming  >> 8);
  BUFFER_APPEND(abtPacketBuffer, PN53X_REG_CIU_BitFraming & 0xff);
  BUFFER_APPEND(abtPacketBuffer, SYMBOL_START_SEND | ((szTxBits % 8) & SYMBOL_TX_LAST_BITS));
  ...
  while (1) {
    ...
    //size_t szRes = sizeof(abtRes);
    // Let's send the previously constructed ReadRegister command
    if ((res = pn53x_transceive(pnd, abtPacketBuffer, BUFFER_SIZE(abtPacketBuffer), abtPacketBuffer2, PN53x_EXTENDED_FRAME__DATA_MAX_LEN, -1)) < 0) {
      return res;
    }
    for (i = 0; i < sz; i++) {
      pbtRx[i + szRxBits] = abtPacketBuffer2[i + off];
    }
    szRxBits += (size_t)(sz & SYMBOL_FIFO_LEVEL);
    sz = abtPacketBuffer2[sz + off];
    if (sz == 0)
      break;
  }
...
pn53x_initiator_transceive_bytes_timed(struct nfc_device *pnd, const uint8_t *pbtTx, const size_t szTx, uint8_t *pbtRx, const size_t szRx, uint32_t *cycles)
{
  ...
  BUFFER_APPEND(abtPacketBuffer, PN53X_REG_CIU_BitFraming  >> 8);
  BUFFER_APPEND(abtPacketBuffer, PN53X_REG_CIU_BitFraming & 0xff);
  BUFFER_APPEND(abtPacketBuffer, SYMBOL_START_SEND);
  // Let's send the previously constructed WriteRegister command
  if ((res = pn53x_transceive(pnd, abtPacketBuffer, BUFFER_SIZE(abtPacketBuffer), NULL, 0, -1)) < 0) {
    return res;
  }
  ...
  while (1) {
    ...
    if (pbtRx != NULL) {
      ...
      // Copy the received bytes
      for (i = 0; i < sz; i++) {
        pbtRx[i + szRxLen] = abtPacketBuffer2[i + off];
      }
    }
  ...
  if (pnd->bCrc) {
    ...
    uint8_t *pbtTxRaw;
    pbtTxRaw = (uint8_t *) pvPortMalloc(szTx + 2);
    if (!pbtTxRaw)
      return NFC_ESOFT;
...
pn53x_target_receive_bits(struct nfc_device *pnd, uint8_t *pbtRx, const size_t szRxLen, uint8_t *pbtRxPar)
{
  ...
  //uint8_t  abtRx[PN53x_EXTENDED_FRAME__DATA_MAX_LEN];
  size_t  szRx = sizeof(abtPacketBuffer2);
  int res = 0;

  // Try to gather a received frame from the reader
  if ((res = pn53x_transceive(pnd, abtCmd, sizeof(abtCmd), abtPacketBuffer2, szRx, -1)) < 0)
    return res;
  szRx = (size_t) res;
  // Get the last bit-count that is stored in the received byte
  uint8_t ui8rcc;
...
pn53x_target_receive_bytes(struct nfc_device *pnd, uint8_t *pbtRx, const size_t szRxLen, int timeout)
{
  ...
  size_t szRx = PN53x_EXTENDED_FRAME__DATA_MAX_LEN;//sizeof(abtRx);
  int res = 0;
  if ((res = pn53x_transceive(pnd, abtCmd, sizeof(abtCmd), abtPacketBuffer, szRx, timeout)) < 0)
    return pnd->last_error;
  szRx = (size_t) res;
  ...
  // Copy the received bytes
  memcpy(pbtRx, abtPacketBuffer + 1, szRx);
...
pn53x_target_send_bits(struct nfc_device *pnd, const uint8_t *pbtTx, const size_t szTxBits, const uint8_t *pbtTxPar)
{
  ...
  abtPacketBuffer[0] = TgResponseToInitiator;
  int res = 0;
  ...
  if ((res = pn53x_transceive(pnd, abtPacketBuffer, szFrameBytes + 1, NULL, 0, -1)) < 0)
    return res;
...
pn53x_target_send_bytes(struct nfc_device *pnd, const uint8_t *pbtTx, const size_t szTx, int timeout)
{
  ...
  // Try to send the bits to the reader
  if ((res = pn53x_transceive(pnd, abtPacketBuffer, szTx + 1, NULL, 0, timeout)) < 0)
    return res;
...
pn53x_InDeselect(struct nfc_device *pnd, const uint8_t ui8Target)
{
  if (CHIP_DATA(pnd)->type == RCS360) {
    ...
    size_t  szStatus = PN53x_EXTENDED_FRAME__DATA_MAX_LEN;//sizeof(abtStatus);
    uint8_t  abtCmdGetStatus[] = { GetGeneralStatus };
    int res = 0;
    if ((res = pn53x_transceive(pnd, abtCmdGetStatus, sizeof(abtCmdGetStatus), abtPacketBuffer, szStatus, -1)) < 0) {
      return res;
    }
    szStatus = (size_t) res;
    if ((szStatus < 3) || (abtPacketBuffer[2] == 0)) {
      return NFC_SUCCESS;
    }
                 ...
                 const uint8_t btPollNr, const uint8_t btPeriod, nfc_target *pntTargets, const int timeout)
{
  ...
  //uint8_t  abtRx[PN53x_EXTENDED_FRAME__DATA_MAX_LEN];
  size_t  szRx = PN53x_EXTENDED_FRAME__DATA_MAX_LEN;//sizeof(abtRx);
  int res = pn53x_transceive(pnd, abtCmd, szTxInAutoPoll, abtPacketBuffer, szRx, timeout);
  szRx = (size_t) res;
  if (res < 0) {
    return res;
  } else if (szRx > 0) {
    szTargetFound = abtPacketBuffer[0];
    if (szTargetFound > 0) {
      uint8_t ln;
      uint8_t *pbt = abtPacketBuffer + 1;
      /* 1st target */
      // Target type
                   ...
                   const int timeout)
{
  ...
  //uint8_t  abtCmd[67] = { InJumpForDEP, (ndm == NDM_ACTIVE) ? 0x01 : 0x00 };
	abtPacketBuffer[0] = InJumpForDEP;
	abtPacketBuffer[1] = (ndm == NDM_ACTIVE) ? 0x01 : 0x00;
  ...
  switch (nbr) {
    case NBR_106:
    	abtPacketBuffer[2] = 0x00; // baud rate is 106 kbps
      break;
    case NBR_212:
    	abtPacketBuffer[2] = 0x01; // baud rate is 212 kbps
      break;
    case NBR_424:
    	abtPacketBuffer[2] = 0x02; // baud rate is 424 kbps
      break;
    case NBR_847:
    case NBR_UNDEFINED:
      pnd->last_error = NFC_EINVARG;
  ...
  if (pbtPassiveInitiatorData && (ndm == NDM_PASSIVE)) {        /* can't have passive initiator data when using active mode */
    switch (nbr) {
      case NBR_106:
    	  abtPacketBuffer[3] |= 0x01;
        memcpy(abtPacketBuffer + offset, pbtPassiveInitiatorData, 4);
        offset += 4;
        break;
      case NBR_212:
      case NBR_424:
    	  abtPacketBuffer[3] |= 0x01;
        memcpy(abtPacketBuffer + offset, pbtPassiveInitiatorData, 5);
        offset += 5;
        break;
      case NBR_847:
      case NBR_UNDEFINED:
  ...
  if (pbtNFCID3i) {
	  abtPacketBuffer[3] |= 0x02;
    memcpy(abtPacketBuffer + offset, pbtNFCID3i, 10);
    offset += 10;
  }

  if (szGBi && pbtGBi) {
	  abtPacketBuffer[3] |= 0x04;
    memcpy(abtPacketBuffer + offset, pbtGBi, szGBi);
    offset += szGBi;
  }
  ...
  if ((res = pn53x_transceive(pnd, abtPacketBuffer, offset, abtPacketBuffer2, szRx, timeout)) < 0)
    return res;
  szRx = (size_t) res;
  // Make sure one target has been found, the PN53X returns 0x00 if none was available
  if (abtPacketBuffer2[1] >= 1) {
    // Is a target struct available
    if (pnt) {
      pnt->nm.nmt = NMT_DEP;
      pnt->nm.nbr = nbr;
      pnt->nti.ndi.ndm = ndm;
      memcpy(pnt->nti.ndi.abtNFCID3, abtPacketBuffer2 + 2, 10);
      pnt->nti.ndi.btDID = abtPacketBuffer2[12];
      pnt->nti.ndi.btBS = abtPacketBuffer2[13];
      pnt->nti.ndi.btBR = abtPacketBuffer2[14];
      pnt->nti.ndi.btTO = abtPacketBuffer2[15];
      pnt->nti.ndi.btPP = abtPacketBuffer2[16];
      if (szRx > 17) {
        pnt->nti.ndi.szGB = szRx - 17;
        memcpy(pnt->nti.ndi.abtGB, abtPacketBuffer2 + 17, pnt->nti.ndi.szGB);
      } else {
        pnt->nti.ndi.szGB = 0;
      }
    }
  }
  return abtPacketBuffer2[1];
}
                     ...
                     uint8_t *pbtRx, const size_t szRxLen, uint8_t *pbtModeByte, int timeout)
{
  ...
  int res = 0;
  abtPacketBuffer2[0] = TgInitAsTarget;

  // Clear the target init struct, reset to all zeros
  memset(abtPacketBuffer2 + 1, 0x00, sizeof(abtPacketBuffer2) - 1);

  // Store the target mode in the initialization params
  abtPacketBuffer2[1] = ptm;

  // MIFARE part
  if (pbtMifareParams) {
    memcpy(abtPacketBuffer2 + 2, pbtMifareParams, 6);
  }
  // FeliCa part
  if (pbtFeliCaParams) {
    memcpy(abtPacketBuffer2 + 8, pbtFeliCaParams, 18);
  }
  // DEP part
  if (pbtNFCID3t) {
    memcpy(abtPacketBuffer2 + 26, pbtNFCID3t, 10);
  }
  // General Bytes (ISO/IEC 18092)
  if ((CHIP_DATA(pnd)->type == PN531) || (CHIP_DATA(pnd)->type == RCS360)) {
    if (szGBt) {
      memcpy(abtPacketBuffer2 + 36, pbtGBt, szGBt);
      szOptionalBytes = szGBt;
    }
  } else {
	  abtPacketBuffer2[36] = (uint8_t)(szGBt);
    if (szGBt) {
      memcpy(abtPacketBuffer2 + 37, pbtGBt, szGBt);
    }
  ...
  if ((CHIP_DATA(pnd)->type != PN531) && (CHIP_DATA(pnd)->type != RCS360)) { // PN531 does not handle Historical Bytes
	  abtPacketBuffer2[36 + szOptionalBytes] = (uint8_t)(szTkt);
    if (szTkt) {
      memcpy(abtPacketBuffer2 + 37 + szOptionalBytes, pbtTkt, szTkt);
    }
  ...
  //uint8_t  abtRx[PN53x_EXTENDED_FRAME__DATA_MAX_LEN];
  size_t szRx = PN53x_EXTENDED_FRAME__DATA_MAX_LEN;//sizeof(abtRx);
  if ((res = pn53x_transceive(pnd, abtPacketBuffer2, 36 + szOptionalBytes, abtPacketBuffer, szRx, timeout)) < 0)
    return res;
  szRx = (size_t) res;
...
pn53x_current_target_new(const struct nfc_device *pnd, const nfc_target *pnt)
{
  ...
  // this code has been commented out as target data is now static
#ifndef NFC_STATIC_STRUCTS
  if (CHIP_DATA(pnd)->current_target) {
	  vPortFree(CHIP_DATA(pnd)->current_target);
  }
  CHIP_DATA(pnd)->current_target = pvPortMalloc(sizeof(nfc_target));
  if (!CHIP_DATA(pnd)->current_target) {
    return NULL;
  }
  memcpy(CHIP_DATA(pnd)->current_target, pnt, sizeof(nfc_target));
#else
  CHIP_DATA(pnd)->current_target = pnt;
#endif
...
__BSS(RAM3) static struct pn53x_data chip_data;

void *
pn53x_data_new(struct nfc_device *pnd, const struct pn53x_io *io)
{
#ifndef NFC_STATIC_STRUCTS
  pnd->chip_data = pvPortMalloc(sizeof(struct pn53x_data));
  if (!pnd->chip_data) {
    return NULL;
  }
#else
  ...
  CHIP_DATA(pnd)->supported_modulation_as_initiator = pn53x_supported_modulation_as_initiator;
...
pn53x_data_free(struct nfc_device *pnd)
{
  ...
  if (CHIP_DATA(pnd)->supported_modulation_as_initiator) {
	  vPortFree(CHIP_DATA(pnd)->supported_modulation_as_initiator);
  }
  vPortFree(pnd->chip_data);
#endif
}
