 ...
 * 
 * $Id: mifare_desfire.c 974 2012-01-25 09:58:16Z adiacre@il4p.fr $
 */
...
#define DESFIRE_TRANSCEIVE2(tag, msg, msg_len, res) \
    do { \
	uint8_t __msg[MAX_CAPDU_SIZE + 5] = { 0x90, 0x00, 0x00, 0x00, 0x00, /* ..., */ 0x00 }; \
...
{
    MifareTag tag;
    if ((tag= pvPortMalloc (sizeof (struct mifare_desfire_tag)))) {
	MIFARE_DESFIRE (tag)->last_picc_error = OPERATION_OK;
	MIFARE_DESFIRE (tag)->last_pcd_error = OPERATION_OK;
...
{
	vPortFree (MIFARE_DESFIRE (tag)->session_key);
	vPortFree (MIFARE_DESFIRE (tag)->crypto_buffer);
	vPortFree (tag);
}
...
mifare_desfire_get_application_ids (MifareTag tag, MifareDESFireAID *aids[], size_t *count)
{
    ...
    *aids = pvPortMalloc (sn - 1);
    if (!(*aids = pvPortMalloc ((*count + 1) * sizeof (MifareDESFireAID))))
	return -1;
...
mifare_desfire_get_df_names (MifareTag tag, MifareDESFireDF *dfs[], size_t *count)
{
    ...
    *dfs = NULL;
	...
	if (__res_n > 1) {
	    ...
	    if ((new_dfs = pvPortRealloc (*dfs, sizeof (*new_dfs) * (*count + 1)))) {
...
mifare_desfire_get_iso_file_ids (MifareTag tag, uint16_t *files[], size_t *count)
{
    ...
    *count = sn / 2;
    *files = pvPortMalloc (sizeof (**files) * *count);
    if (!*files)
	return -1;
