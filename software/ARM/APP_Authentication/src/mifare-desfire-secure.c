...
int
mifare_desfire_initialize_secure(MifareTag tag, uint16_t filesize, uint8_t *key_data_app, uint16_t access_flags, uint32_t aid)
{
...

    {
		...
		res = mifare_desfire_create_std_data_file(tag,0x00,MDCM_PLAIN,access_flags,filesize);
		if (res < 0)
...
int
mifare_desfire_write_secure(MifareTag tag, char * buffer, uint16_t filesize, uint32_t aid, uint8_t *key_data)
{
	uint8_t *cc_data;
	uint8_t *ndef_msg;
    int error = EXIT_SUCCESS;
...

    {
		...
		if (res < 0)
		errx (EXIT_FAILURE, "Authentication with NDEF Tag Application master key failed");
		res = mifare_desfire_write_data(tag,0x00,0,filesize,buffer);
...
int
mifare_desfire_read_secure(MifareTag tag, char * buffer, uint16_t filesize, uint32_t aid, uint8_t *key_data)
{
	uint8_t *cc_data;
	uint8_t *ndef_msg;
    int error = EXIT_SUCCESS;
...

    {
		...
		if (res < 0)
		errx (EXIT_FAILURE, "Authentication with NDEF Tag Application master key failed");
		res = mifare_desfire_read_data(tag,0x00,0,filesize,buffer);
